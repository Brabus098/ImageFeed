...
final class ImagesListPresenterTests: XCTestCase {
    ...
    func test_didSelectImage_withValidURL_callsPresentSingleImageViewer() {
        ...
        let photo = Photo(id: "1", size: .zero, createdAt: Date(), welcomeDescription: "", thumbImageURL: "", largeImageURL: "https://example.com/image.jpg", isLiked: false)
    ...
    func test_toggleLike_failure_doesNotCrashOrReload() {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
            ...
            XCTAssertFalse(presenter.photos[0].isLiked) // remains unchanged
            expectation.fulfill()
        }
